...
const getOneCustomer = async () => {
    try {
        ...
        const response = await api.request('get', `/api/getOneCustomer/${props.id}`, null, tokenUser);
        if (response.error) {
            router.push("../");
        } else {
...
onMounted(() => {
    ...
    getOneCustomer(props.id);
});
const customerUpdateByAdmin = async () => {
    try {
        ...
        const tokenUser = localStorage.getItem('accessToken');
        const response = await api.request('put', `/api/updateCustomer/ ${props.id}`, updatedData, tokenUser);
    ...
    <div class="bg-[#c4b9b9] min-h-screen h-full  bg-opacity-75">
        <div class="flex justify-center">
            <div class="bg-white w-fit p-7 mt-40 ml-60 mr-60 rounded-lg">
                ...
                <form @submit.prevent="customerUpdateByAdmin">
