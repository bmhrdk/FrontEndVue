...
const getOneService = async () => {
    try {
        ...
        if (response.error) {
            ...
            technician.value = response.technician;
...
const updateOneServiceId = async () => {
    try {
        ...
        const finalStatus = status.value === 'Menunggu Pembayaran' ? 'Selesai' : (status.value === 'Diproses' ? status.value : newStatus.value);
        ...
        const updatedData = {
            ...
            merek: merek.value,
            tipe: tipe.value,
            // status: status.value === 'Diproses' ? status.value : newStatus.value,
            status: finalStatus,
            estimasi: estimasi.value,
            ...(newStatus.value === 'Menunggu Pembayaran' && {
                processedId: adminId._value,
            }),
            ...(status.value === 'Menunggu Pembayaran' && {
                finishedId: adminId._value,
            }),
...
function addKerusakan() {
    if (kerusakanList.value.length < 5) {
...
const historyService = ref(false);

const toggleHistoryService = () => {
    historyService.value = !historyService.value;
};
        ...
        <div class="bg-white p-1 my-20 mx-32 rounded-lg">
            ...
            <form @submit.prevent="updateOneServiceId">
                <div class="text-sm grid items-center grid-cols-3 gap-y-8 gap-x-10 px-6 ">
                    ...
                    <div v-if="status !== 'Diproses'" class="space-y-2 col-span-3">
                        <div class="flex">
                            <label for="kerusakan" class="font-medium">Kerusakan & Perbaikan:</label>
                            <button
                                v-if="kerusakanList.length < 5 && status != 'Selesai' && status != 'Pengerjaan Selesai' && status != 'Menunggu Pembayaran'"
                    ...
                    <div v-if="status !== 'Diproses'" class="space-y-2 col-span-3">
                        <label v-if="spareparts.lenght > 0" for="tipe" class="font-medium">Sparepart Tambahan:
                    ...
                    <div v-if="status !== 'Selesai'" class="p-0.5 w-80 space-y-2 ml-auto">
                    ...
                    <div v-if="status !== 'Selesai' && status !== 'Menunggu Pembayaran' && status !== 'Diproses'"
                        class="flex justify-center items-center col-span-3 space-x-2">
                        ...
                        <input v-if="status !== 'Selesai' && status !== 'Menunggu Pembayaran'" type="radio"
                            v-model="newStatus" id="pengerjaanselesai" value="Pengerjaan Selesai">
                    ...
                    <div v-else-if="status === 'Selesai'" class="flex justify-center items-center col-span-3">
                        <div class=" border-b-2 border-green-500 text-2xl text-green-500">SERVIS SELESAI </div>
                    </div>
                </div>
                <div class="flex space-x-2 justify-center">
                    <button v-if="status === 'Menunggu Pembayaran'" type="submit"
                        ...
                        Selesaikan Transaksi Servis
                    </button>
                    <button v-else-if="status != 'Selesai'" type="submit"
                        class="mt-8 text-sm flex items-center space-x-2 font-normal text-white bg-greenprimary rounded-md p-2 px-3 hover:bg-white hover:outline hover:outline-greenprimary hover:text-greenprimary transition-all duration-100">
                        <IconCheck class="mr-2" />
                        Edit Selesai
                    </button>
                    <button v-else-if="status === 'Selesai'" @click.prevent="toggleHistoryService"
                        class="mt-8 text-sm flex items-center space-x-2 font-normal text-white bg-greenprimary rounded-md p-2 px-3 hover:bg-white hover:outline hover:outline-greenprimary hover:text-greenprimary transition-all duration-100">
                        <IconCheck class="mr-2" />
                        History
                    </button>
                    <button v-if="status === 'Selesai' || status === 'Menunggu Pembayaran'" @click.prevent="notaPDF"
                        class="mt-8 text-sm flex items-center space-x-2 font-normal text-white bg-greenprimary rounded-md p-2 px-3 hover:bg-white hover:outline hover:outline-greenprimary hover:text-greenprimary transition-all duration-100">
                        <IconPDF class="w-5 h-5 mr-2" />
                        Download PDF
                    </button>
                </div>
                <div v-if="historyService"
                    class="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-30 z-50">
                    <div class="bg-white shadow-lg rounded-xl ml-64 p-6">
                        <div class=" flex pt-0 pb-2">
                            <button @click="toggleHistoryService"
                        ...
                        <div class="border-t-2 border-red-500 pt-6">
                            <div class="grid grid-cols-1 gap-4">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex">
                                       <span class="mr-2 uppercase text-green-500">{{role}}</span>
                                       <div class="capitalize">{{createdBy}}</div>
                                    </div>
                                </div>
                                <div class="space-y-2">
                                    <div class="font-semibold">
