...
const getOneInventory = async () => {
    try {
        ...
        const response = await api.request('get', `/api/getOneInventory/${props.id}`, null, tokenUser);
        if (response.error) {
            router.push("../");
        } else {
...
onMounted(() => {
    ...
    getOneInventory(props.id);
});
const updateOneInventoryId = async () => {
    try {
        ...
        const tokenUser = localStorage.getItem('accessToken');
        const response = await api.request('put', `/api/updateInventory/ ${ props.id }`, updatedData, tokenUser);
...
<template>
    ...
    <div class="flex justify-center">
        <div class="bg-white w-fit p-7 mt-40 ml-60 mr-60 rounded-lg">
            ...
            <form @submit.prevent="updateOneInventoryId">
                <div class="text-sm items-center">
                    <div class="space-y-2 mb-2">
                        <label for="merek" class="font-medium">Merek:</label>
                        <input id="merek" type="text" v-model="merek"
                            class="uppercase p-2 px-4 w-80 flex rounded-lg bg-blurwhite ring-inset ring-1 ring-graydark focus:ring-graydark" />
                    </div>
                    <div class="space-y-2 mb-2">
                        <label for="tipe" class="font-medium">Tipe:</label>
                        <input id="tipe" type="text" v-model="tipe"
                            class="capitalize p-2 px-3 w-80 flex rounded-lg bg-blurwhite ring-inset ring-1 ring-graydark focus:ring-graydark" />
                    </div>
                    <div class="space-y-2 mb-2">
                        <label for="stok" class="font-medium">Stok:</label>
                        <input id="stok" type="number" v-model="stok"
                            class="p-2 px-3 w-80 flex rounded-lg bg-blurwhite ring-inset ring-1 ring-graydark focus:ring-graydark" min="0" />
                    </div>
                    <div class="space-y-2 mb-2">
